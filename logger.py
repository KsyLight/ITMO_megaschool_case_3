...
def save_log(
    ...
    for turn in log.get("turns", []):
        ...
        # Используем \n для разделения мыслей разных агентов
        final_thoughts_str = "\n".join(thoughts_str_parts)

        # Строгий порядок полей для JSON
        export_data["turns"].append({
            "turn_id": turn["turn_id"],
            "agent_visible_message": turn["agent_visible_message"], # Вопрос
            "user_message": turn["user_message"], # Ответ
            "internal_thoughts": final_thoughts_str # Мысли
        })
